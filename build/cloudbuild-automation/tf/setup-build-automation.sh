...
REPOSITORY_OWNER="psx95" # Replace this with the name of owner of the repository
...
function terraform_init() {
    docker run -v $(pwd):/app -w /app -e "GOOGLE_APPLICATION_CREDENTIALS=${GOOGLE_APPLICATION_CREDENTIALS}" \
        -v "${GOOGLE_APPLICATION_CREDENTIALS}:${GOOGLE_APPLICATION_CREDENTIALS}:ro" hashicorp/terraform:latest init
    
    docker run -v $(pwd):/app -e "GOOGLE_APPLICATION_CREDENTIALS=${GOOGLE_APPLICATION_CREDENTIALS}" \
        -v "${GOOGLE_APPLICATION_CREDENTIALS}:${GOOGLE_APPLICATION_CREDENTIALS}:ro" \
        -w /app hashicorp/terraform:latest apply \
